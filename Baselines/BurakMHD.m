function [PD, PF, Precision, F1, AUC, Accuracy, G_measure, MCC, Balance, probPos] = BurakMHD(sources,target,LOC,k)
% function [PD, PF, Precision, F1, AUC, Accuracy, G_measure, MCC, Balance, Popt, Popt20, IFA, probPos] = BurakMHD(sources,target,LOC,k)
...
%     % Use metric ranges of source dataset to discretize target dataset 
%     for j=1:nFeature
%         edges = filterDiscSrc.getCutPoints(j-1);
%         target(:,j) = discretize(target(:,j), [-inf, edges', inf], 'IncludedEdge', 'left');
%     end
%     tars{i} = target;
    

%     wekaTar = mat2ARFF(tars{i}, 'classification'); % self-defined function 'mat2ARFF'
